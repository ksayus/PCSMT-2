...
class Examine:
    def IsExistAdminAccount(account_name=None):
        """
        检查管理员账户文件完整性
            ...
            if account_name == None:
                ...
                if accounts != None:
                    for account_name in accounts:
                        ...
                        if Examine.IsExistAdminAccount(account_name):
                            pass
                        else:
    ...
    def Password(account, password):
        ...
        :return: False(错误) or True(正确)
        """
        if Examine.IsExistAdminAccount(account):
            try:
                with open("C:\PCSMT2-key\\account\\" + account + ".bin", "rb") as f:
                    password_plain = Key.Do.DecipherKey(f.read())
...
class Set:
    def AdminAccount(account, password):
        """
        设置管理员账户
        :param account: 管理员账户名称
        :param password: 管理员密码
            ...
            if Examine.IsExistAdminAccount(account):
                ...
                try:
                    with open("C:\PCSMT2-key\\account\\" + account + ".bin", "wb") as f:
                        password = Key.Do.CipherKey(password)
                        f.write(password)
                except Exception as e:
