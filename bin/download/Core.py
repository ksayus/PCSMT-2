...
from bin.download import CoreInstaller
...
class Processing:
    def DownloadCore(server_name, core_type, core_support_version):
        """
        下载服务器核心
        ...
        #fabric核心
        if core_type == 'fabric':
            if CoreInstaller.Core.InitInstaller(core_type, core_support_version):
                time.sleep(3)
                try:
                    log.logger.info('正在下载Fabric核心...')
                    os.system('powershell java -jar ' + Info.work_path + Info.Core.Position.fabric_core_installation_save + ' server -mcversion ' + core_support_version + ' -downloadMinecraft -dir "' + Info.work_path + Info.File.Folder.Servers + '\\' + server_name + '"')
                    core_path_default = Path(Info.work_path + Info.File.Folder.Servers + '\\' + server_name + Info.Core.DefaultName.Fabric)
                    if core_path_default.exists():
                        core_path_default.rename(Info.work_path + Info.File.Folder.Servers + '\\' + server_name + '\\' + server_name + '.jar')
        ...
        if core_type == 'forge':
            if CoreInstaller.Core.InitInstaller(core_type, core_support_version):
                ...
                try:
                    ...
                    os.system('java -jar ' + Info.work_path +Info.Core.Position.forge_core_installation_save + ' --installServer "' + Info.work_path + Info.File.Folder.Servers + '\\' + server_name + '"')
                    os.remove(Info.work_path + Info.Core.Position.forge_core_installation_save)
        ...
        if core_type == 'official':
            try:
                ...
                os.system('powershell curl -o '+ Info.work_path + Info.File.Folder.Servers + '\\' + server_name + Info.Core.DefaultName.Official + ' ' + server_download_url)
        ...
        if core_type == 'mohist':
            try:
                ...
                log.logger.info('当前下载链接:' + server_download_url)
                os.system('powershell curl -o '+ Info.work_path + Info.File.Folder.Servers + '\\' + server_name + Info.Core.DefaultName.Mohist + ' ' + server_download_url)
                return True
            except Exception as e:
