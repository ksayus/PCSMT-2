...
class Core:
    def InitInstaller(core_type, core_support_version):
        """
        初始化核心下载器
        ...
        """
        if core_type == 'fabric':
            if find_file.find_files_with_existence(Info.work_path + Info.Core.Position.fabric_core_installation_save):
                ...
                try:
                    ...
                    os.system('powershell curl -o '+ Info.work_path + Info.Core.Position.fabric_core_installation_save + ' https://maven.fabricmc.net/net/fabricmc/fabric-installer/1.0.1/fabric-installer-1.0.1.jar')
        ...
        if core_type == 'forge':
            try:
                ...
                try:
                    os.system('powershell curl -o '+ Info.work_path + Info.Core.Position.forge_core_installation_save + ' https://maven.minecraftforge.net/net/minecraftforge/forge/'+ core_support_version + '-' + forge_core_version + '/forge-' + core_support_version + '-' + forge_core_version + '-installer.jar')
                    log.logger.info('下载完成')
                    return True
                except Exception as e:
